...
for i = 1:numRows
    for j = 1:numCols
        ...
        if ~isempty(edge_sub) || ~isempty(gray_sub) || ~isempty(color_sub)
            ...
            filename = sprintf('%s\\enh\\subImg_%d_%d.jpg', currentFolder, i, j);
...
matFilename = fullfile(currentFolder, 'enh.mat');
save(matFilename, 'T');
