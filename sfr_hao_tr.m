%sfr_hao_tr Edge Spatial Frequency Response (eSFR) test chart
%
%   An sfr_hao_tr encapsulates the Extended/Enhanced/WedgeExtended/WedgeEnhanced
%   versions of the edge spatial frequency response test chart specified in 
%   ISO 12233:2014 standard.
%
% sfr_hao_tr Create an sfr_hao_tr object from an image.
%
%   CHART = sfr_hao_tr(A) creates an sfr_hao_tr object CHART
%   from image A. A can be an RGB or grayscale image.
%
%   CHART = sfr_hao_tr(___, Name, Value, ___) constructs an
%   sfr_hao_tr object CHART where additional parameters
%   controlling the chart import can be provided as name value
%   pairs in case the import is unsuccessful using the default
%   parameter settings.
%
%   Parameters are:
%
...
%                             is 0.5.
%
%   'RegistrationPoints'    - Coordinates of the four registration 
%                             points, specified as a 4x2 matrix 
%                             where row one corresponds to the top
%                             left, two to the top right, three
%                             to the bottom right, and four to
%                             the bottom left registration
%                             point.
%
%   NOTE: Only one of the above two name value pairs can be
%         specified while importing a test image and not both.
%
%   'Style'                 - Style of test chart used, specified
%                             as "Extended", "Enhanced", "WedgeExtended",
%                             or "WedgeEnhanced". By default,  
%                             sfr_hao_tr performs automatic chart
%                             style detection. Specify this argument 
%                             if the detection is unsuccessful.
%
%   'CameraParameters'      - Camera parameters to compensate for
%                             distortion, specified as a cameraParameters
%                             object (requires Computer Vision 
%                             ToolboxTM).
%
%   'RefinePoints'          - Refine model points when determining
%                             SlantedEdgeROIs, specified as a 
%                             logical scalar true (default) or 
%                             false. To replicate results from 
%                             releases R2020b and prior, specify
...
%   -----
%
%   1.  The sfr_hao_tr is an extended, enhanced, wedge-extended or  
%       wedge-enhanced version of 12233:2014 standard test chart. 
%       Check references for further details.
...
%
%   [2] Imatest eSFR ISO test chart
%
%   Example 1
%   ---------
%   % This example shows how to construct an sfr_hao_tr object from a test
%   % image. Post construction the chart object is displayed for visually
%   % verifying proper import.
%
%   I = imread('eSFRTestImage.jpg');
%   chart = sfr_hao_tr(I,'Sensitivity',0.6);
%   figure
%   displayChart(chart);
%
%   Example 2
%   ---------
%   % This example shows how to construct an sfr_hao_tr object by providing the
...
%   imshow(I)
%   [X, Y] = ginput(4);
%   chart = sfr_hao_tr(I,'RegistrationPoints',[X, Y]);
%   figure
%   displayChart(chart);
%
%   See also sfr_hao_tr, measureSharpness, measureChromaticAberration,
%            measureNoise, measureColor, measureIlluminant
%
% Class Properties and Methods:
%
%   sfr_hao_tr properties:
...
%                                         particular chart.
%       Style                           - Style of test chart used. Specified as        
%                                         'Extended', 'Enhanced', 'WedgeExtended' or 'WedgeEnhanced'.
%
%
%   sfr_hao_tr methods:
%       sfr_hao_tr                   - Construct an sfr_hao_tr object from a test image.
...
%       displayChart                - Display constructed sfr_hao_tr
%
%
...
classdef sfr_hao_tr
    ...
    properties(SetAccess = private, GetAccess = public)
        ...
        %
        %   Original chart image provided to create the sfr_hao_tr object to
        %   perform quality measurements. It is an RGB or grayscale image.
        Image
        ...
        %Style - Style of test chart used.
        % 
        %   A string specifying the style of Imatest test chart used. 
        %   Specified as  "Extended", "Enhanced", "WedgeExtended", or "WedgeEnhanced".
        Style
    end
    ...
    methods
        function chart = sfr_hao_tr(varargin)
            %   CHART = sfr_hao_tr(A) creats an sfr_hao_tr object CHART
            %   from image A. A can be an RGB or grayscale image.
            %
            %   CHART = sfr_hao_tr(___, Name, Value, ___) constructs an
            %   sfr_hao_tr object CHART where additional parameters
            ...
            if nargin > 0
                ...
                if size(options.A,3)==3
                    chart.Image = options.A;
                else
                    chart.Image = repmat(options.A,[1,1,3]);
                end
                sensitivity = options.Sensitivity;
                RegistrationPoints = options.RegistrationPoints;
                Style = options.Style;
                CameraParameters=options.CameraParameters;
                RefinePoints=options.RefinePoints;
                ...
                if ~chart.importUsingRegistrationPoints
                    ...
                    if invalidRegistrationPoints
                        error(message('images:sfr:invalidRegistrationPoints'));
                    end                    
                end
                
                
                if isempty(CameraParameters)
                    [chart.RegistrationPoints, chart.modelPoints, chart.Style] = images.internal.testchart.register_esfrChart(chart, chart.importSensitivity,RegistrationPoints,Style);
                    images.internal.testchart.verify_esfrChart(chart);
                else
                    [chart.RegistrationPoints, chart.modelPoints, chart.Style] = estimateDistortedChart(chart,CameraParameters,RegistrationPoints,Style);
                    images.internal.testchart.verify_esfrChart(chart);
                end
                

                [chart.refinedPoints, chart.SlantedEdgeROIs] = images.internal.testchart.detectSlantedEdgeROIs(chart,RefinePoints);
                
                %{
                for i = [1,2,3,4,21,22,23,24,41,42,43,44,17,18,19,20,37,38,39,40,57,58,59,60]
                    chart.SlantedEdgeROIs(i).ROI = [NaN,NaN,NaN,NaN];
                    chart.SlantedEdgeROIs(i).ROIIntensity = [];
                end
                %}
        ...
        function noiseTable = measureNoise(chart)
            %measureNoise   Measure noise using sfr_hao_tr
            %
            %   noiseTable = measureNoise(CHART) measures noise levels
            %   using the gray ROIs in a sfr_hao_tr image.
            %
            ...
            %   Class Support
            %   -------------
            %   CHART is an sfr_hao_tr object. noiseTable is a table.
            %
            %   Notes
            %   -----
            %
            ...
            %       on test image data.
            %   2.  An sfr_hao_tr should be correctly imported for accurate and reliable
            %       measurements. Use sfr_hao_tr function to create a correctly imported
            %       chart.
            %
            %   Example
            %   -------
            ...
            %   grid on
            %
            %   See also sfr_hao_tr, measureColor, measureSharpness,
        ...
        function [colorTable, varargout] = measureColor(chart)
            %measureColor   Measure color reproduction using sfr_hao_tr
            %
            %   colorTable = measureColor(CHART) measures color values of
            %   the color ROIs in an sfr_hao_tr image.
            %
            ...
            %                         reference L*a*b* color values as outlined
            %                         in CIE76
            %
            %   Class Support
            %   -------------
            %
            %   CHART is an sfr_hao_tr object. colorTable is a table.
            %   colorCorrectionMatrix is a 4-by-3 double matrix.
            %
            %   Notes
            %   -----
            %
            %   1.  An sfr_hao_tr should be correctly imported for accurate and
            %       reliable measurements. Use sfr_hao_tr function to create a
            %       correctly imported chart.
            %   2.  The colorCorrectionMatrix is a 4-by-3 matrix which
            ...
            %   Example 2
            %   ---------
            %   % This example shows the procedure for calculating and
            ...
            %   figure
            %   displayChart(chart);
            %   [~, colorCorrectionMatrix] = measureColor(chart);
            %
            %   C = imapplymatrix(colorCorrectionMatrix(1:3,:)', rgb2lin(I), colorCorrectionMatrix(4,:));
            %   C = lin2rgb(C);
            %   figure, imshow(C), title('Color corrected image')
            %
            %   Example 3
            %   ---------
            %   % This example improves the result from example 2 by
            %   % including the gray patches as well as the color patches
            %   % in the least squares fit.
            %
            %   I = imread('eSFRTestImage.jpg');
            %   chart = sfr_hao_tr(I);
            %   CT = measureColor(chart);
            %
            ...
            %
            %   measuredGrayRGB = zeros(20, 3, 'uint8');
            %   for idx = 1:20
            %       R = mean2(rgb2lin(chart.GrayROIs(idx).ROIIntensity(:,:,1)));
            %       G = mean2(rgb2lin(chart.GrayROIs(idx).ROIIntensity(:,:,2)));
            %       B = mean2(rgb2lin(chart.GrayROIs(idx).ROIIntensity(:,:,3)));
            %       measuredGrayRGB(idx,:) = uint8([R G B]);
            %   end
            %
            %   measuredRGB = [rgb2lin([CT.Measured_R CT.Measured_G CT.Measured_B]); measuredGrayRGB];
            %
            %   mat = double([measuredRGB ones(36,1)]) \ double(referenceRGB);
            %
            %   Iadjust = imapplymatrix(mat(1:3,:)', rgb2lin(I), mat(4,:)');
            %   Iadjust = lin2rgb(Iadjust);
            %   figure; imshow(Iadjust); title('Corrected with both gray and color patches')
            %
            %
            %   See also sfr_hao_tr, measureNoise, measureSharpness,
            %   measureChromaticAberration, rgb2lin, lin2rgb,
            ...
            nargoutchk(1,2);
            validateChart(chart);
            if (isequal(chart.Image(:,:,1),chart.Image(:,:,2),chart.Image(:,:,3)))
                warning(message('images:sfr_hao_tr:grayChartColorMetrics'));
            end
            
            colorTable = images.internal.testchart.esfrMeasureColor(chart);
            if nargout == 2
                measured_RGB = rgb2lin([colorTable.Measured_R colorTable.Measured_G colorTable.Measured_B]);
                
                colorCorMatrix = images.internal.testchart.calculateColorCorMatrix(measured_RGB, ...
                    lab2rgb(chart.ReferenceColorLab,OutputType=class(colorTable.Measured_R),ColorSpace="linear-rgb"));
        ...
        function aberrationTable = measureChromaticAberration(chart, varargin)
            %measureChromaticAberration     Measure chromatic aberration using sfr_hao_tr
            %
            ...
            %   -------------
            %
            %   CHART is an sfr_hao_tr object. ROIIndex can be uint8,
            ...
            %   7.  An sfr_hao_tr should be correctly imported for accurate and reliable
            %       measurements. Use sfr_hao_tr function to create a
            %       correctly imported chart.
            %
            %   Example
            %   -------
            %   % This example show how to measure chromatic aberration using sfr_hao_tr
            ...
            %
            %   See also sfr_hao_tr, measureSharpness, measureNoise, measureColor
            ...
            if isempty(validROIs)
                error(message('images:sfr_hao_tr:noSlantedEdgeROIs'));
            end
        ...
        function [sharpnessTable, varargout] = measureSharpness(chart, varargin)
            %measureSharpness Compute spatial frequency response using sfr_hao_tr
            %
            %   sharpnessTable = measureSharpness(CHART) measures spatial
            %   frequency response (SFR) at all slanted edge regions of
            %   interest (ROIs) of an sfr_hao_tr. Additional measurements
            ...
            %                           same as displayed when an sfr_hao_tr is displayed
            ...
            %   -------------
            %
            %   CHART is an sfr_hao_tr object. ROIIndex and PercentResponse
            ...
            %   6.  An sfr_hao_tr should be correctly imported for accurate and reliable
            %       measurements. Use sfr_hao_tr function to create a
            %       correctly imported chart.
            %
            %   References
            ...
            %   % using sfr_hao_tr and plot results.
            %
            %   I = imread('eSFRTestImage.jpg');
            %   I = rgb2lin(I);
            %   chart = sfr_hao_tr(I);
            ...
            %
            %   See also sfr_hao_tr, measureColor, measureNoise, measureChromaticAberration,
            ...
            %sharpnessTable = MeasureSharpness(chart,validROIs,percentResponse);
        ...
        function illum = measureIlluminant(chart)
            % measureIlluminant Estimate the scene illuminant using sfr_hao_tr
            %
            %   ILLUMINANT = measureIlluminant(CHART) measures the scene
            %   illuminant using the gray ROIs of the sfr_hao_tr
            %
            %   Class Support
            %   -------------
            %
            %   CHART is an sfr_hao_tr object. ILLUMINANT is a three element
            %   vector of doubles.
            %
            %   Notes
            %   -----
            ...
            %   3.  An sfr_hao_tr should be correctly imported for accurate
            %       and reliable measurements. Use sfr_hao_tr function to
            %       create a correctly imported chart.
            %
            %   Example
            %   -------
            %   % This example shows how to estimate the illuminant of the
            %   % scene using sfr_hao_tr and how to white balance an image
            ...
            %
            %   See also sfr_hao_tr, chromadapt, measureColor, measureNoise,
        ...
        % Display functions
        function pic = displayChart(varargin)
            %displayChart Display sfr_hao_tr with overlaid regions of interest
            %
            %   displayChart(CHART) displays a valid sfr_hao_tr once it has
            %   been successfully imported.
            %
            %   displayChart(___, Name, Value, ___) displays a valid
            %   sfr_hao_tr with additional parameters controlling aspects of
            %   the chart display.
            %
            %   Parameters are:
            %
            ...
            %   -------------
            %
            %   CHART is an sfr_hao_tr object.
            %
            %   Example
            %   -------
            %   % This example shows how to import and display an sfr_hao_tr object
            %
            %   I = imread('eSFRTestImage.jpg');
            %   chart = sfr_hao_tr(I);
            %
            ...
            %   displayChart(chart)
            %
            %   See also sfr_hao_tr, measureSharpness, measureNoise,
            %   measureColor
            
            narginchk(1,11);
            options = parseInputsDisplayChart(varargin{:});
            chart = options.chart;
            displayEdgeROIs = logical(options.displayEdgeROIs);
            displayGrayROIs = logical(options.displayGrayROIs);
            displayColorROIs = logical(options.displayColorROIs);
            displayRegistrationPoints = logical(options.displayRegistrationPoints);
            ...
            pic = draw(chart, displayEdgeROIs, displayGrayROIs, displayColorROIs, parentAxis);
        end
...
parser.addParameter('Sensitivity',[],@validateSensitivity);
parser.addParameter('RegistrationPoints',[],@validateRegistrationPoints);
parser.addParameter('Style', [],@validateStyle);
parser.addParameter('CameraParameters',[],@validateCameraParameters)
parser.addParameter('RefinePoints',true,@validateRefinePoints)

parser.parse(varargin{:});
options = parser.Results;
%Sensitivity and RegistrationPoints cannot be provided together
if (~isempty(options.RegistrationPoints) & ~isempty(options.Sensitivity))
    error(message('images:sfr_hao_tr:invalidArgumentPair'));
end
if (isempty(options.Sensitivity))
    options.Sensitivity=0.5;
end
if (~isempty(options.Style))
    options.Style = validatestring(options.Style,{'Extended','Enhanced',...
        'WedgeEnhanced','WedgeExtended'},mfilename,'Style');
end
end
...
validColorImage = ((ndims(A) == 3) && (size(A,3) == 3)) || (ndims(A) == 2);
if ~validColorImage
    error(message('images:sfr_hao_tr:invalidImageFormat','A'));
end

[row, col, ~] = size(A);
sizeFlag = (row >= 20) && (col >= 20);
if ~sizeFlag
    error(message('images:sfr_hao_tr:smallImageSize','A'));
end
end
...
function B = validateStyle(Style)

validatestring(Style,{'Extended','Enhanced','WedgeEnhanced','WedgeExtended'},...
    mfilename,'Style');
...
function B = validateCameraParameters(cameraParams)

checkCVT=license('test', 'Video_and_Image_Blockset');
if (checkCVT == 0)
    error(message('images:sfr_hao_tr:noInstallCVT'));
end
attributes = {'scalar'};
supportedClasses = {'cameraParameters'};
validateattributes(cameraParams,supportedClasses, attributes, mfilename,'CameraParameters');
B = true;
end

function B = validateRefinePoints(RefinePoints)

validateattributes(RefinePoints,{'logical'},{'scalar'},...
    mfilename,'RefinePoints');
...
function [RegistrationPoints, modelPoints, Style]=estimateDistortedChart(chartUndistorted,CameraParameters,RegistrationPoints,Style)
    %undistorted image 
    [chartUndistorted.Image, newOrigin] = undistortImage(chartUndistorted.Image,CameraParameters,'OutputView','full');
    chartUndistorted.ImageGray = rgb2gray(chartUndistorted.Image);
    [chartUndistorted.ImageRow, chartUndistorted.ImageCol, ~] = size(chartUndistorted.Image);
    chartUndistorted.sigma = ceil(chartUndistorted.ImageCol/500);

    undistortedRegistrationPoints=[];
    if chartUndistorted.importUsingRegistrationPoints
        %transform registration points to undistorted image
        undistortedRegistrationPoints=undistortPoints(RegistrationPoints,CameraParameters)-newOrigin;
    end
    %detect chart in undistorted image
    [chartUndistorted.RegistrationPoints, chartUndistorted.modelPoints, chartUndistorted.Style] = images.internal.testchart.register_esfrChart(chartUndistorted, chartUndistorted.importSensitivity,undistortedRegistrationPoints,Style);
    images.internal.testchart.verify_esfrChart(chartUndistorted);
    %transform estimated points to original image
    modelPoints=round(CameraParameters.distortPoints(chartUndistorted.modelPoints+newOrigin));
    RegistrationPoints=round(CameraParameters.distortPoints(chartUndistorted.RegistrationPoints+newOrigin));
    Style=chartUndistorted.Style;
end
